...
diode_dir = os.path.join(scratch_dir, "lab-simulators", "diode-lab-simulator")
gas_dir = os.path.dirname(os.path.abspath(__file__))

# 1. Read styles.css
with open(os.path.join(diode_dir, "styles.css"), "r", encoding="utf-8") as f:
    css_content = f.read()

# 2. Read simulator.js
with open(os.path.join(diode_dir, "simulator.js"), "r", encoding="utf-8") as f:
    sim_content = f.read()

# 3. Read index.html structure
with open(os.path.join(diode_dir, "index.html"), "r", encoding="utf-8") as f:
    html = f.read()

# 4. Read ui.js
with open(os.path.join(diode_dir, "ui.js"), "r", encoding="utf-8") as f:
    ui_content = f.read()
...
function submitReportToGAS() {
  ...
  let ledState = '';
  const lsRadios = document.getElementsByName('led-state');
  for (let r of lsRadios) {
    if (r.checked) ledState = r.value;
  }
  ...
  const data = {
    ...
    diodeDirection: STATE.diodeDirection,
    ...
    ledState: ledState,
    vD: document.getElementById('v-d-val').value.trim(),
    vR: document.getElementById('v-r-val').value.trim(),
    vLed: document.getElementById('v-led-val').value.trim(),
    vSum: document.getElementById('v-sum-val').value.trim(),
    iCalc: document.getElementById('i-calc-val').value.trim(),
    iMeas: document.getElementById('i-meas-val').value.trim(),
  ...
  if (typeof google !== 'undefined' && google.script && google.script.run) {
    ...
    setTimeout(() => {
      onSuccessGrading({
        ...
        feedback: '1.1 ความต้านทานไบอัสตรง: ถูกต้อง\\n1.2 ความต้านทานไบอัสกลับ: ถูกต้อง\\n1.3 ระบุสรุปสภาพไดโอด: ถูกต้อง\\n2.1 สถานะการส่องสว่างของ LED: ถูกต้อง\\n2.2 แรงดัน VD: ถูกต้อง\\n2.3 แรงดัน VR: ถูกต้อง\\n2.4 แรงดัน VLED: ถูกต้อง\\n2.5 ผลรวมแรงดัน (KVL): ถูกต้อง\\n2.6 กระแสคำนวณ Icalc: ถูกต้อง\\n2.7 กระแสวัดจริง Imeas: นอกเกณฑ์'
      });
    }, 1500);
  }
}
...
html = html.replace(
    ...
    '''<div class="footer-buttons-row" style="display: flex; gap: 15px; flex-wrap: wrap; justify-content: center; width: 100%;">
            <button class="btn btn-secondary btn-large" onclick="printReport()">🖨️ พิมพ์รายงานผลการทดลอง / บันทึก PDF</button>
...
# Embed images as Base64 if needed, but since diode_lab_banner.png is in the same directory, 
# wait! Google Sheets Apps Script cannot serve diode_lab_banner.png locally.
# Let's check if we should embed the image as Base64 in index.html to make it truly standalone!
# Let's convert diode_lab_banner.png to base64.
import base64
banner_path = os.path.join(diode_dir, "diode_lab_banner.png")
