...
mosfet_dir = os.path.join(scratch_dir, "lab-simulators", "mosfet-pinout-lab-simulator")
gas_dir = os.path.dirname(os.path.abspath(__file__))

# 1. Read styles.css
with open(os.path.join(mosfet_dir, "styles.css"), "r", encoding="utf-8") as f:
    css_content = f.read()

# 2. Read simulator.js
with open(os.path.join(mosfet_dir, "simulator.js"), "r", encoding="utf-8") as f:
    sim_content = f.read()

# 3. Read index.html structure
with open(os.path.join(mosfet_dir, "index.html"), "r", encoding="utf-8") as f:
    html = f.read()

# 4. Read ui.js
with open(os.path.join(mosfet_dir, "ui.js"), "r", encoding="utf-8") as f:
    ui_content = f.read()
...
function submitReportToGAS() {
  ...
  // Package table 1 rows (6 rows)
  const part1Rows = [];
  for (let i = 0; i < 6; i++) {
    part1Rows.push({
      deflect: document.getElementById('nch-deflect-' + i).value
    });
  }
  ...
  for (let i = 0; i < 6; i++) {
    part2Rows.push({
      deflect: document.getElementById('pch-deflect-' + i).value
    });
  }
  
  // Package table 3 rows (6 rows)
  const part3Rows = [];
  for (let i = 0; i < 6; i++) {
    part3Rows.push({
      deflect: document.getElementById('t3-deflect-' + i).value
    });
  }
  ...
  const data = {
    ...
    ansModel1Type: document.getElementById('ans-model1-type').value,
    ansModel2Type: document.getElementById('ans-model2-type').value,
  ...
  if (typeof google !== 'undefined' && google.script && google.script.run) {
    ...
    setTimeout(() => {
      onSuccessGrading({
        ...
        score: 10,
        maxScore: 10,
        comment: 'ผ่านเกณฑ์ดีเยี่ยม (Local Simulation)',
        feedback: 'ตารางที่ 1 (หาขา IRF540): ถูกต้อง 6 จาก 6 แถว (ได้ 2.00 คะแนน)\nตารางที่ 2 (หาขา IRF9540): ถูกต้อง 6 จาก 6 แถว (ได้ 2.00 คะแนน)\nตารางที่ 3 (การทดสอบกระตุ้นเกต): ถูกต้อง 6 จาก 6 ขั้นตอน (ได้ 4.00 คะแนน)\nส่วนระบุขาและชนิดสาร: ถูกต้อง 5 จาก 5 คำถาม (ได้ 2.00 คะแนน)'
      });
    }, 1500);
  }
}
